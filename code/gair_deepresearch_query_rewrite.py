...
TEMPORAL_REWRITE_PROMPT = """You rewrite user questions into search queries for Present-Anchored Temporal QA (PAT-QA).

Today: {today}
As-of date for retrieval: {as_of}

Rules:
- Keep the original intent EXACTLY.
- Make the query time-scoped using "as of {as_of}" (or "currently" if appropriate).
- Disambiguate entities using short descriptors when helpful (occupation, country, film director, etc.).
- Output 3-4 diverse queries:
  (1) general authoritative
  (2) wikipedia-style / encyclopedic
  (3) news-oriented (recent)
  (4) official/government if relevant (e.g., "president.gov.ua", ".gov", "official website")
- Avoid overly long queries (<= 18 words each).
...
def temporal_query_rewrite_node(state: ResearchState) -> ResearchState:
    state = dict(state)
    state.setdefault("trace", [])

    # If planner already provided explicit queries, don't rewrite unless you want to override
    tool_args = state.get("tool_args") or {}
    existing = tool_args.get("query")
    if isinstance(existing, list) and any(str(q).strip() for q in existing):
        state["rewritten_queries"] = [str(q).strip() for q in existing if str(q).strip()]
        state["rewrite_reason"] = "planner_provided_queries"
        state["rewrite_used_as_of"] = state.get("search_as_of") or now_ymd()
        return state

    question = state["main_question"]
    as_of = state.get("search_as_of") or now_ymd()

    # Cache (avoids repeated LLM cost)
    payload = json.dumps({"q": question, "as_of": as_of}, sort_keys=True)
    cache_key = _sha(payload)
    cache = state.setdefault("rewrite_cache", {})  # small in-memory cache per run

    if cache_key in cache:
        obj = cache[cache_key]
        state["rewritten_queries"] = obj.get("queries", []) or [question]
        state["rewrite_reason"] = obj.get("reason", "cache_hit")
        state["rewrite_used_as_of"] = obj.get("as_of", as_of)
        state["trace"].append({"node": "query_rewrite", "cache_hit": True, "n": len(state["rewritten_queries"])})
        return state

    prompt = TEMPORAL_REWRITE_PROMPT.format(
        today=now_ymd(),
        as_of=as_of,
        question=question,
    )
    ...
    try:
        ...
        if not queries:
            queries = [question]
        # hard cap to avoid planner->search explosion
        queries = queries[:4]

        cache[cache_key] = {"queries": queries, "reason": obj.get("reason",""), "as_of": obj.get("as_of", as_of)}
        state["rewritten_queries"] = queries
        state["rewrite_reason"] = obj.get("reason", "llm_rewrite")
        state["rewrite_used_as_of"] = obj.get("as_of", as_of)
        state["trace"].append({"node": "query_rewrite", "cache_hit": False, "queries": queries})
    except Exception:
        # fallback
        state["rewritten_queries"] = [f"{question} as of {as_of}", question]
        state["rewrite_reason"] = "rewrite_parse_failed_fallback"
        state["rewrite_used_as_of"] = as_of
        state["trace"].append({"node": "query_rewrite", "cache_hit": False, "error": "json_parse"})

    # Feed rewritten queries into the existing search tool_args
    state["tool_args"] = {"query": state["rewritten_queries"]}
    return state
...
#     g.add_node("query_rewrite", temporal_query_rewrite_node)

#     # route: planner(search) -> query_rewrite -> search
#     g.add_conditional_edges("planner", route_from_planner, {
#         "search": "query_rewrite",
#         "browse": "browse",
#         "final": "final",
#     })
#     g.add_edge("query_rewrite", "search")
