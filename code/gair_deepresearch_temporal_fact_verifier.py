...
def _tokenize(s: str) -> list[str]:
    # simple tokenizer; swap to your own
    return re.findall(r"[A-Za-z0-9]+", (s or "").lower())

# def fact_to_doc(f: dict) -> str:
#     # Make BM25 “see” the useful fields
#     parts = [
#         f.get("subject",""),
#         f.get("relation",""),
#         f.get("object",""),
#         f.get("start_date","") or "",
#         f.get("end_date","") or "",
#         f.get("justification","") or "",
#         f.get("evidence_hint","") or "",
#     ]
...
#         self.facts = facts
#         self.facts_flat, self.docs = flatten_temporal_facts_jsonl("/home/csgrad/jmeem001/langgraph_scratch/gpt-4-min-agent/dec15/output/parellel_temporal_facts_store.jsonl")

#         # self.docs = [fact_to_doc(f) for f in facts]
#         self.corpus = [tokenize_fast(d) for d in docs]#[_tokenize(d) for d in self.docs]
#         self.bm25 = BM25Okapi(self.corpus)

#     def search(self, query: str, k: int = 25) -> list[dict]:
...
#         for i in idxs:
#             f = dict(self.facts[i])
#             f["_bm25_score"] = float(scores[i])
#             out.append(f)
#         return out

def build_fact_doc(f: Dict[str, Any]) -> str:
    # Local variables = faster than repeated dict lookups
    subj = f.get("subject")
    rel  = f.get("relation")
    obj  = f.get("object")
    sd   = f.get("start_date")
    ed   = f.get("end_date")
    hint = f.get("evidence_hint")

    # Keep it short (BM25 likes signal > noise)
    # Include relation + entity + object + dates + hint
    parts = (
        fast_clean(subj),
        fast_clean(rel),
        fast_clean(obj),
...
class TemporalFactBM25:
    ...
    def search(self, query: str, k: int = 25) -> list[dict]:
        ...
        out = []
        for i in idxs:
            f = dict(self.facts[i])              # ✅ use the flattened list aligned to docs
            f["_bm25_score"] = float(scores[i])
            out.append(f)
        return out


def format_facts_for_prompt(facts: list[dict], max_items: int = 15) -> str:
    if not facts:
        return "None"
    lines = []
    for i, f in enumerate(facts[:max_items], 1):
        lines.append(
            f"{i}. subj={f.get('subject')} | rel={f.get('relation')} | obj={f.get('object')} | "
            f"start={f.get('start_date')} | end={f.get('end_date')} | conf={f.get('confidence')} | "
            f"hint={f.get('evidence_hint')} | bm25={f.get('_bm25_score'):.3f}"
        )
    return "\n".join(lines)
...
I will provide you with some retrieved relevant temporal facts that may help you answer the question.
## Your task
...
yes or no.
If the retrieved_temporal_facts contain sufficient evidence to answer, output <answer>...</answer>.
If facts are missing/insufficient, choose web search or browse webpage.
# Tools
...
def persist_trace(state: ResearchState, run_id: Optional[str] = None):
    ...
    path = TRACE_DIR / f"trace_run_temp_verify.json"
...
# def fact_retrieve_node(state: ResearchState) -> ResearchState:
#     state = dict(state)
#     state.setdefault("trace", [])
#     state.setdefault("context_so_far", "None")

#     # Choose retrieval query:
#     # 1) last_search_queries (most recent intent)
#     # 2) else main_question
#     q = state["main_question"]
#     lsq = state.get("last_search_queries") or []
    
#     if lsq:
#         q = q + " | " + lsq[-1]
#     if state["context_so_far"]:
#         q = q + " | " + state["context_so_far"]
    
#     # q = state["main_question"]
#     state["fact_query"] = q

#     # Retrieve
#     retriever: TemporalFactBM25 = state["_fact_retriever"]  # inject at init
#     facts = retriever.search(q, k=10)

#     # Optional: filter by confidence threshold
#     # facts = [f for f in facts if float(f.get("confidence", 0) or 0) >= 0.5]

#     state["retrieved_facts"] = facts
#     state["retrieved_facts_text"] = format_facts_for_prompt(facts, max_items=15)
...
#     state.setdefault("trace", [])
#     state.setdefault("context_so_far", "None")
#     state.setdefault("retrieved_facts", [])
#     state.setdefault("retrieved_facts_text", "None")

#     retriever: TemporalFactBM25 = state["_fact_retriever"]  # injected at init
#     k = 10

#     # 1) Base query = main question (+ last intent)
#     q_parts = [state["main_question"]]
#     lsq = state.get("last_search_queries") or []
#     if lsq:
#         q_parts.append(lsq[-1])

#     # 2) Evidence delta (NOT full context)
#     # Prefer a dedicated field you update in browse_node when you append extracted info.
#     delta = (state.get("last_extracted_info") or "").strip()
#     if delta:
#         q_parts.append(delta[:500])  # cap

#     q = " | ".join(q_parts)
#     state["fact_query"] = q

#     # 3) Cache key: only changes when intent/delta changes
#     cache_payload = {
#         "q_base": state["main_question"],
#         "last_intent": (lsq[-1] if lsq else ""),
#         "delta_hash": _hash_str(delta[:1500]) if delta else "",
#         "k": k,
#         # add other retriever settings here if they matter (as_of, index name, etc.)
#     }
#     cache_key = _hash_str(json.dumps(cache_payload, sort_keys=True))

#     prev_key = state.get("fact_cache_key")
#     if prev_key == cache_key and state.get("retrieved_facts") is not None:
#         state["trace"].append({
#             "node": "fact_retrieve",
#             "skipped": True,
#             "reason": "cache_hit",
...
#     # 4) Retrieve
#     facts = retriever.search(q, k=k)

#     state["retrieved_facts"] = facts
#     state["retrieved_facts_text"] = format_facts_for_prompt(facts, max_items=15)
#     state["fact_cache_key"] = cache_key
...
def planner_node(state: ResearchState) -> ResearchState:
    ...
    if state["step_count"] >= state.get("max_steps", 15):
        ...
        prompt = RESEARCH_PLANNER_PROMPT.format(
            ...
            retrieved_facts_text=state.get("retrieved_facts_text", "None"),
        )
        out = llm.invoke([HumanMessage(content=prompt)]).content
    ...
    for url in url_list:
        ...
        while True:
            ...
            if extracted.strip():
                ...
                state["last_extracted_info"] = extracted.strip()
            else:
                state["context_so_far"] += f"\n\n[No new info]\nurl={url}\npage_index={page_index}/{total_pages-1}"
...
def make_fact_retrieve_node(retriever: TemporalFactBM25):
    def fact_retrieve_node(state: ResearchState) -> ResearchState:
        ...
        facts = retriever.search(q, k=k)
        state["retrieved_facts"] = facts
        state["retrieved_facts_text"] = format_facts_for_prompt(facts)
    ...
    return fact_retrieve_node



def build_app():
    g = StateGraph(ResearchState)
    # g.add_node("fact_retrieve", fact_retrieve_node)
    g.add_node("fact_retrieve", make_fact_retrieve_node(FACT_RETRIEVER))
    ...
    g.set_entry_point("fact_retrieve")
    g.add_edge("fact_retrieve", "planner")

    g.add_edge("search", "fact_retrieve")
    g.add_edge("browse", "fact_retrieve")
...
# facts_flat, docs = flatten_temporal_facts_jsonl("/home/csgrad/jmeem001/langgraph_scratch/gpt-4-min-agent/dec15/output/parellel_temporal_facts_store.jsonl")
# optionally: add_doc_field_inplace(facts_flat)



# Pre-tokenize corpus once:
# corpus = [tokenize_fast(d) for d in docs]
...
if __name__ == "__main__":
    ...
    for q in tqdm(data):
        init: ResearchState = {
            ...
            "max_steps": 10,
            "trace": [],
            # "_fact_retriever": TemporalFactBM25("/home/csgrad/jmeem001/langgraph_scratch/gpt-4-min-agent/dec15/output/parellel_temporal_facts_store.jsonl")
        }
